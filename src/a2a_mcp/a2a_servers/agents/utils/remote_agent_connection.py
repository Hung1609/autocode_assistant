# Overview: 
# RemoteAgentConnections is act as an intermediate layer to communicate with agent through A2AClient
# support both streaming and non-streaming, along with callback to handle update


import uuid
from typing import Callable

from a2a_servers.common.a2a_client import A2AClient
...
class RemoteAgentConnections: # --> hold the connections to the remote agents.
    def __init__(self, agent_card: AgentCard):
        print(f"Connecting to agent: {agent_card.name} at {agent_card.url}")
        self.agent_client = A2AClient(agent_card) # create A2AClient with agent_card
        print(self.agent_client.url)
        self.card = agent_card
        self.conversation_name = None
        self.conversation = None
        self.pending_tasks = set() # set of task's ID are waiting to handle

    def get_agent(self) -> AgentCard:
        return self.card

    async def send_task(self, request: TaskSendParams, task_callback: TaskUpdateCallback | None,) -> Task | None:
...
def merge_metadata(target, source): # copy metadata from source to target
